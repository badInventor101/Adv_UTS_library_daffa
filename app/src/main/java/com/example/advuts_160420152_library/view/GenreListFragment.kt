...
class GenreListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val recGen = view.findViewById<RecyclerView>(R.id.recViewGen)
        recGen.layoutManager = LinearLayoutManager(context)
        recGen.adapter = genreListAdapter
    ...
    fun observeViewModel() {   /// di lihat dan di amati
        viewModel.booksLD.observe(viewLifecycleOwner, Observer {
            genreListAdapter.updateGenreList(it) // lari ke studentListAdapter, di update studentListnya
        })
    }
